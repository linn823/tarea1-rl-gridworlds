...
class GridWorld(gym.Env):
    ...
    def __init__(self, file_name = "map2.txt", fail_rate = 0.0, terminal_reward = 2.0, move_reward = 0.0, bump_reward = -0.5, bomb_reward = -1.5):
        ...
        self.viewer.window = pyglet.window.Window(width=self.viewer.width, height=self.viewer.height, display=self.viewer.display, vsync=False, resizable=True) # con esto se arregla la ventana
        self.n = None
        self.m = None
        self.bombs = []
        self.walls = []
    ...
    def take_action(self, action):
        ...
        #rand = np.random.rand()
        #if rand < 0.1: # probabilidad 0.1 irse a la izq
        #    action = LEFT
        #elif rand < 0.2: # probabilidad 0.1 irse a la der
        #    action = RIGHT
        ## probabilidad .8 continuar con la intencion original
